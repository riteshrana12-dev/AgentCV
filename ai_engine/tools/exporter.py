...
def _is_section_heading(value: str) -> bool:
    ...
    return key in SECTION_NAMES or (
        value.strip().isupper() and 2 <= len(value.split()) <= 5
    )
...
def parse_resume_text(
    ...
    if tailored_bullets:
        ...
        additions = [
            ...
            if _clean_line(bullet).lower() not in existing
        ]

        experience.lines = additions + experience.lines

    # IMPORTANT:
    # Do not sort sections here.
    # The original resume structure should remain in its
    # original order when this parser is used.
    return ExportResumeData(
        name=name,
        contact=contact,
        sections=sections,
...
def parse_tailored_structure(
    tailored_structure: dict,
    candidate_name: str | None = None,
) -> ExportResumeData:
    """
    Convert the AI-generated tailored resume structure directly
    into ExportResumeData.

    The AI-generated section order is preserved exactly.

    Multiple sections with the same title are also preserved.
    For example, multiple "Projects" sections will all be
    rendered instead of one overwriting another.
    """

    sections: list[ResumeSection] = []

    for section in tailored_structure.get("sections", []):
        ...
        lines = [
            _clean_line(str(line))
            for line in raw_lines
            if _clean_line(str(line))
        ]
    ...
    return ExportResumeData(
        name=(
            tailored_structure.get("name")
            or candidate_name
            or "Candidate"
        ),
        contact=str(
            tailored_structure.get("contact", "")
        ).strip(),
        sections=sections,
    )
...
def export_docx(
    ...
    *,
    ...
    # If AI-generated structured data is provided,
    # use it directly.
    #
    # Only fall back to parse_resume_text() when structured
    # data is not provided.
    data = data or parse_resume_text(
        raw_resume or "",
        bullets,
        candidate_name,
    )
...
def export_pdf(
    ...
    *,
    ...
    one_page_canvas = type(
        "OnePageCanvas",
        (_OnePageCanvas, Canvas),
        {},
    )

    document.build(
        story,
        canvasmaker=one_page_canvas,
    )
...
class _OnePageCanvas:
    ...
    def showPage(self):  # noqa: N802 - ReportLab API name
        ...
        if self._pageNumber > 2:
            raise ValueError(
                "Resume content exceeds the one-page export limit."
            )
